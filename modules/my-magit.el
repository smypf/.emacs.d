...
(defun magit-open-pull-request ()
  ...
  (save-excursion
    ...
    (when
        (search-backward-regexp "https://" nil t)
      (browse-url-at-point))))
...
(use-package magit
  ...
  ((magit-process-finish-apply-ansi-colors t)
   ...
   (git-commit-summary-max-length (symbol-value 'fill-column))
   ;; Display magit as a full window when opened
   ;; This doesn't always work the way I want it to.
   (magit-display-buffer-function 'magit-display-buffer-fullframe-status-v1)
   ;; Restore window configuration after closing magit
   ;; This is useful with the above change
   (magit-bury-buffer-function 'magit-restore-window-configuration)
   ;; Don't show Contained / Merged / Whatever.
   ;; https://github.com/magit/magit/discussions/5105
   ;; https://stackoverflow.com/questions/65596406/meaning-of-contained-and-merged-in-magit
  ...
  :config
  ;; Automatically save all modified buffers when entering magit
  ;; This prevents the annoying save prompts during magit operations
  (defun smypf/magit-auto-save-buffers ()
    "Automatically save all modified buffers when entering magit."
    (save-some-buffers t))

  ;; Alternative: Track modified buffers and optionally restore them
  (defvar smypf/magit-modified-buffers nil
    "List of buffers that were modified before entering magit.")

  (defun smypf/magit-track-modified-buffers ()
    "Track which buffers are modified before entering magit."
    (setq smypf/magit-modified-buffers
          (cl-remove-if-not #'buffer-modified-p (buffer-list))))

  (defun smypf/magit-restore-modified-buffers ()
    "Restore the modified state of buffers that were modified before magit."
    (dolist (buffer smypf/magit-modified-buffers)
      (when (buffer-live-p buffer)
        (with-current-buffer buffer
          (set-buffer-modified-p t)))))

  ;; Uncomment the lines below if you want to track instead of auto-save
  ;; (add-hook 'magit-status-mode-hook #'smypf/magit-track-modified-buffers)
  ;; (add-hook 'magit-status-mode-exit-hook #'smypf/magit-restore-modified-buffers)

  (add-hook 'magit-status-mode-hook #'smypf/magit-auto-save-buffers)

  ;; Remove `magit-blame-echo` from M-x menu
  (setopt read-extended-command-predicate #'command-completion-default-include-p)
  (put 'magit-blame-echo 'completion-predicate #'ignore)
...
(defun my-pbcopy-kill-ring ()
  "Copy the latest entry from the kill ring to the system clipboard using pbcopy."
  (interactive)
  (let ((latest-kill (current-kill 0 t)))
    (shell-command (format "echo %s | pbcopy" (shell-quote-argument latest-kill)))))


(use-package git-link
  :commands smypf/copy-git-link
  :bind ("C-c g l" . smypf/copy-git-link))
...
;; Custom magit entry function with buffer management options
(defun smypf/magit-with-buffer-management (&optional arg)
  "Open magit with configurable buffer management.
With prefix arg, prompt for save strategy.
Without prefix arg, auto-save all modified buffers."
  (interactive "P")
  (let ((modified-buffers (cl-remove-if-not #'buffer-modified-p (buffer-list))))
    (if (null modified-buffers)
        ;; No modified buffers, just open magit
        (magit-status arg)
      ;; There are modified buffers, handle them
      (if arg
          ;; With prefix arg, let user choose
          (let ((choice (read-char-choice
                         "Modified buffers detected. Choose action:
s - Save all buffers
d - Discard all changes
c - Cancel
" '(?s ?d ?c))))
            (pcase choice
              (?s (save-some-buffers t) (magit-status arg))
              (?d (dolist (buf modified-buffers)
                    (when (buffer-live-p buf)
                      (with-current-buffer buf
                        (revert-buffer t t))))
                  (magit-status arg))
              (?c (message "Magit cancelled"))))
        ;; Without prefix arg, auto-save
        (progn
          (save-some-buffers t)
          (magit-status arg))))))
